TW.IDE.Widgets.Customtable = function () {
    ...
    this.renderHtml = function () {
        return '<div class="widget-content widget-Customtable"></div>';
    };
    ...
    this.setupWidget = function () {
        ...
        var jsondocinfomration = typeof widgetProps.JSONDocinformation == "string" ?  JSON.parse(widgetProps.JSONDocinformation) : widgetProps.JSONDocinformation
        ...
        var jsonheader_information = typeof widgetProps.JSONHeaderinformation== "string" ?  JSON.parse(widgetProps.JSONHeaderinformation)  :widgetProps.JSONHeaderinformation
    ...
    //  var jsondashboard_information =  typeof widgetProps.JSONDashboardinformation == "string" ?  JSON.parse(widgetProps.JSONDashboardinformation) :widgetProps.JSONDashboardinformation
    ...
    //     }
        ...
        var imageexport = d3
            ...
            .on("click", function (d, i) {
                if (booleanpdf && booleanexcell) {
                    converHTMLToCanvas("testTable", file_name, true);
                } else if (!booleanpdf && !booleanexcell) {
                } else if (!booleanexcell) {
                    converHTMLToCanvas("testTable", file_name);
                } else {
                    tableToExcel("testTable", "worksheet", file_name);
                }
            });

        makeTable("testTable");

        function makeTable(idtable) {
            var table = d3
                .select(`#${widgetID}`)
                .append("table")
                .attr("cellspacing", "0")
            ...
            var rowheader1 = tbody
                .append("tr")
                .attr("id", "header_table" + idtable);
            rowheader1
                .append("td")
                .attr("rowspan", function () {
                    if (doc_information.length > 0)
                        return doc_information.length;
                    return 1;
                })
            ...
            var row_header_information = Math.ceil(
                header_information.length / 5
            );
            ...
            while (count_row_header_information <= row_header_information) {
                ...
                header_information
                    ...
                    .forEach(
                        (element_header_information, i_header_information) => {
                            countcolumn += 3;
                            rowheader1
                                .append("td")
                                .attr("height", "21")
            ...
            var row_dashboard_information = Math.ceil(
                dashboard_information.length / 5
            );
            var count_row_dashboard_information = 1;

            while (
                count_row_dashboard_information <= row_dashboard_information
            ) {
                var rowheader1 = tbody.append("tr");
                for (let i = 0; i < colgroupdata.length; i++) {
                    if (i == 0) {
        ...
        function tableToExcel(tableold, name, file_name) {
            ...
            link.click();
            d3.selectAll(".row_data_table" + tableold).remove();

            d3.select("#image_hansell").text("");
            d3.select("#image_buttom").text("");
            d3.select("#image_hansell")
                .append("img")
                .attr("id", "image_hansell_img" + tableold)
                .attr("src", "")
                .attr("width", "0")
                .attr("height", "0")
                .attr("hspace", "0")
            ...
            d3.select("#image_buttom")
                .append("img")
                .attr("id", "img_image_button" + tableold)
                .attr("src", "")
                .attr("width", "0")
                .attr("height", "0")
                .attr("hspace", "0")
        ...
        function converHTMLToCanvas(
            ...
            var sizeheightpdfmax = heightvar - 30;
            var sizeheightpdf = ((widthvar - 30) * height) / width;
            var sizeheighdata =
                (23 * ((widthvar - 30) * height)) / width / height;
            ...
            if (sizeheightpdf > sizeheightpdfmax) {
                ...
                // d3.selectAll("rect.negative").remove()
                d3.selectAll(".row_data_table" + elementold).remove();
                //    $(".row_data_table"+elementold).remove();
                exporttopdf(
                    element,
                    false,
                    true,
        ...
        function exporttopdf(
            ...
            d3.select("#Page").text("Page " + page + " of " + allpage);
                ...
                for (let i = 0; i < colgroupdata.length; i++) {
                    if (i == 0) {
                        ...
                        rowheader1
                            ...
                            .text(function () {
                                if (datatableoke[count_row_data - 1]) {
                                    return datatableoke[count_row_data - 1][
                                        i - 1
                                    ];
                                }
                                return "";
            ...
            html2canvas(element, { scale: 2 }).then(function (canvas) {
                if (booleanAddpage == true) {
                    jspdf.addPage("l", "pt", "a4");
                  
                }

                image = canvas.toDataURL("image/png", 0.1);
                jspdf.addImage(
                    image,
                    "PNG",
                    15,
                    15,
                    widthvar - 30,
                    ((widthvar - 30) * height) / width,
                    null,
                    "FAST"
                ); // A4 sizes

                if (booleanSave == true) {
                    jspdf.save(filenamenew + ".pdf");
                    d3.selectAll(".row_data_table" + elementold).remove();
                    d3.select("#Page").text("Page " + 1 + " of " + 1);
                    if (withexcel) {
                        tableToExcel("testTable", "worksheet", file_name);
                    }
                    d3.select("#image_hansell_img" + elementold)
